...
@Transactional
public class OrderPartDaoTest {

    OrderPart oldEntity;
    OrderPart newEntity;

    @Autowired
    @Qualifier("orderPartDaoImpl")
    private EntityDao<OrderPart> entityDao;

    @Before
    public void before() {
        CreateBefore createBefore = new CreateBefore();
        createBefore.prepareOrderPart();
        newEntity = createBefore.orderPart;
    ...
    @Test
    public void get() {
        Assert.assertEquals("Рулевая рейка", entityDao.get((long) 1).getName());
    }

    @Test
    public void findConcrete() {
        List<OrderPart> list = entityDao.find("рей");
        Assert.assertEquals("Рулевая рейка", list.get(0).getName());
    }
